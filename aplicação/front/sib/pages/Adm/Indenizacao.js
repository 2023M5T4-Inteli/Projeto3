...
function Aprova(iemi) {
  ...
  const data = contrato.methods.aprovarIndenizacao(iemi).encodeABI();
...
export const aprovarIndenizacao = () => {
    ...
    <div className="bg-[#151A20] min-h-screen">
      ...
      <div className='flex flex-row'>
        ...
        <div className='flex flex-col pt-10 mx-10'>
          ...
          <Button text='Aprovar Solicitações' color='bg-[#303030] mt-7 mx-0 text-sm text-white' onclick={red} />
          <Button text='Aprovar Indenizacao' color='bg-[#A5A4FF] mt-7 mx-0 text-sm text-white' />
            ...
            {dados.map(dd => (
              <div key={dd.id} className='w-full bg-[#FFFFFF] flex flex-row mt-7 rounded-xl'>
                <div className='flex flex-col mx-2'>
                  ...
                  <p className='my-1  '>Motivo: {dd[2]}</p>
                  <p className='my-1  '>Valor Solicitado: R$ {dd[1]}</p>
                  <p className='my-1'>IMEI: {dd.IMEI}</p>
                </div>
                <div className='flex flex-row items-start justify-between mt-8'>
                  <div className='bg-[#02DE82] mx-10 w-[100px] h-[60px] text-center' onClick={() => Aprova(dd.IMEI)}>
...
export default aprovarIndenizacao;
